...
from consts import HUMANS, HUMAN_ATTRIBUTES,BACKGROUNDS,RELATIVES, NONLIVING_ATTRIBUTES,NONLIVINGS
from consts import STYLES,FLOWER_INTERACTIONS,FLOWER_ACTIONS,ARTISTS
...
interactions = [
    ...
    'presented by', 'juggled by', 'planted by', "watered by"
]
...
class CaptionGeneratorFlower(CaptionGenerator):
    ...
    def generate_caption(self):
        ...
        if sampled_type=='human_interactions':
            mlm_caption=self.generate_human_interaction_caption()
    ...
    def generate_triplet(self):
        ...
        if sampled_type_pos=='human_interactions':
            anchor=self.generate_human_interaction_caption()
        ...
        if sampled_type_neg=='human_interactions':
            neg=self.generate_human_interaction_caption()
    ...
    def generate_human_interaction_caption(self):
        ...
        prompt=f"<new> is {interaction} by a {subject}"
        return prompt
    ...
    def generate_backgrounds_caption(self):
        ...
        if choice=="type1":
            ...
            activity=np.random.choice(FLOWER_ACTIONS)
            prompt=f"<new> {activity} at {background}"
        return prompt
    def generate_styles_caption(self):
        ...
        if choice=='type1':
            ...
            prompt=f"painting of <new1> by artist {artist}"
        return prompt