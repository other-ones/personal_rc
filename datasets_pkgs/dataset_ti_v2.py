...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.get_images:
            ...
            # keyword idxs
            is_keyword_tokens_mlm=np.zeros_like(input_ids_pos)
            is_keyword_tokens_mlm[np.where(input_ids_pos==self.placeholder_token_id)]=1
            # mlm labels
            mlm_labels=np.ones_like(input_ids_pos)*(-100)
            mlm_labels[masked_idxs]=input_ids_pos[masked_idxs]
            ...
            example["non_special_idxs"]=torch.Tensor(non_special_idxs).bool()
