...
class UNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.resnets[0](hidden_states, temb)
        # print(attn_mod_params is None ,'attn_mod_params is None in midblock') # False
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            hidden_states = attn(
                hidden_states,
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        # print(attn_mod_params,'attn_mod_params_CrossAttnDownBlock2Dc','ATTNCHECK)
        output_states = ()
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.training and self.gradient_checkpointing: #NO
                # print("CKPT")
...
class UpBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, upsample_size=None,attn_mod_params=None):
        for resnet in self.resnets:
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
