...
class StableDiffusionPipelineSyngenSD(StableDiffusionPipeline):
    def __init__(self,
                 vae: AutoencoderKL,
    ...
    def __call__(
        ...
        # attn_mod_params={}
        # tensor([991, 981, 971, 961, 951, 941, 931, 921, 911, 901, 891, 881, 871, 861,
    ...
    def _get_attn_map(
        ...
        for token_lengths in token_lengths_batch:
            ...
            # indices = {i: tok 
            #     for tok, i in zip(self.tokenizer.convert_ids_to_tokens(ids), range(len(ids)))
            # }
            # for key in indices:
            ...
            for aidx,item in enumerate(attention_maps):
                if aidx>=token_lengths+1:
                    break
                # print(torch.max(item),torch.min(item),'item range',item.shape,aidx)
                item=item.detach().cpu().numpy()
                attn_maps.append(item)
                # tmap=keymaps[aidx].detach().cpu().numpy()
                # item=(item*255)
                # tmap=(tmap*255)
                # item=cv2.resize(item,(512,512))
                # tmap=cv2.resize(tmap,(512,512))
                # if is_keyword_tokens[lidx][aidx]:
                #     cv2.imwrite('attention_maps/attn{}_key.png'.format(aidx),item)
                #     cv2.imwrite('attention_maps/tmap{}_key.png'.format(aidx),tmap)
                # else:
                #     cv2.imwrite('attention_maps/attn{}.png'.format(aidx),item)
                #     cv2.imwrite('attention_maps/tmap{}.png'.format(aidx),tmap)
