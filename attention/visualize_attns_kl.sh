...
accelerate launch --main_process_port 2731  visualize_attns_kl.py \
  ...
  --benchmark_path='../datasets_pkgs/eval_prompts/analysis_kl.json' \
  --eval_prompt_type="all" 

export DATA_DIR="/data/twkim/diffusion/personalization/collected/images/dog6/";
export MODEL_NAME="runwayml/stable-diffusion-v1-5";
export CUDA_VISIBLE_DEVICES=6;
accelerate launch --main_process_port 2732  visualize_attns_kl.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --train_data_dir1=$DATA_DIR \
  --placeholder_token1="<dog6>" \
  --resolution=512 \
