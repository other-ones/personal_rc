...
class AttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        if self.cur_att_layer >= 0 and is_cross:
            if attn.shape[1] == np.prod(self.attn_res): #mid layer is dropped because of this
                self.step_store[place_in_unet].append(attn)
            # else:
            #     print(place_in_unet,'AttentionStore mismatched')
            #     print('target res',self.attn_res)
            #     print('curren_res',attn.shape[1])
    ...
    def aggregate_attention(self, from_where: List[str],batch_size,do_classifier_free_guidance) -> torch.Tensor:
        ...
        # FIXED
        outs_ = []
        # print(from_where,'from_where')
        attention_maps = self.get_average_attention() #scores
        for location in from_where:
            # print(attention_maps.keys(),'attention_maps.keys()',attention_maps['mid'],'mid_map') 
            for item in attention_maps[location]: 
                cross_maps = item.reshape(-1, self.attn_res[0], self.attn_res[1], item.shape[-1])
                # print(torch.min(cross_maps),torch.max(cross_maps),'cross_maps',location,'location')
...
class AttendExciteAttnProcessor:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None,attn_mod_params=None):
        ...
        # channel devided by number of heads
        # e.g., 2,4096,320 -> 10,4096,64
        ...
        if attn_mod_params is not None:
            do_attn_mod=attn_mod_params['do_attn_mod']
        if ((attn_mod_params) is not None) and (is_cross):
            treg=attn_mod_params["treg"]
            ...
            # pos_masks_batch.shape: 5,  77,64,64
            # attention_probs:       10, 4096, 77
            ...
            if do_classifier_free_guidance:
                ...
                # cond_scores=cond_scores+((offset_pos*(pos_masks_batch>0)))
                # cond_scores=cond_scores-((offset_neg*(~(pos_masks_batch>0)))*(treg))
                # attention_scores[(batch_size*num_heads):]=cond_scores
                attention_probs=attention_scores.softmax(dim=-1)
                # attention_probs[:,:,15]*=0
                # attention_probs[(batch_size*num_heads):]=cond_probs*(pos_masks_batch)#*treg
                if do_attn_mod:
                    attention_probs[:(batch_size*num_heads)]=attention_probs[:(batch_size*num_heads)]*(pos_masks_batch)
                self.attnstore(attention_probs, is_cross, self.place_in_unet)
                hidden_states = torch.bmm(attention_probs, value)
                hidden_states = attn.batch_to_head_dim(hidden_states)

                # torch.Size([20, 64, 77]) pos_masks_batch.shape
                # torch.Size([40, 64, 77]) attention_scores.shape
                # torch.Size([40, 77, 64]) value.shape
                # torch.Size([40, 64, 64]) hidden_states before
                # torch.Size([2, 64, 1280]) hidden_states after
                # linear proj
