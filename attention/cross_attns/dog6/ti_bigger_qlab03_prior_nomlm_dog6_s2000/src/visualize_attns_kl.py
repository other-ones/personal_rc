...
def main(args):
    ...
    kl_log_file=open(os.path.join(exp_dir,'sym_kl.json'),'w')
    ...
    for batch_idx in range(num_batches):
        ...
        attn_res=256
        kl_criterion = torch.nn.KLDivLoss(reduction='batchmean',log_target=False)
        ...
        for img_idx,img in enumerate(image_list):
            ...
            N=len(bg_maps)
            keyword_maps=keyword_maps.repeat(N,1,1)
