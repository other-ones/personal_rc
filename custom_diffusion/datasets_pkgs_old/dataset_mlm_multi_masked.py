...
class TextualInversionDatasetMulti(Dataset):
    def __init__(
        ...
        print(placeholder_ids,'placeholder_ids')
    ...
    def __getitem__(self, index):
        ...
        if self.get_images:
            ...
            if np.random.rand()<0.5:
                ...
                # random ordering
                if np.random.rand()<0.5:
                    text = random.choice(prefixes).format(placeholder1+' and '+placeholder2)
                else:
                    text = random.choice(prefixes).format(placeholder2+' and '+placeholder1)
            else:
                sampled_concept=np.random.choice([0,1])
        ...
        if np.random.rand()<0.5:
            caption_pos=mlm_pos.format(placeholder1,placeholder2)
        else:
            caption_pos=mlm_pos.format(placeholder2,placeholder1)
        if np.random.rand()<0.5:
            caption_neg=mlm_neg.format(placeholder1,placeholder2)
        else:
            caption_neg=mlm_neg.format(placeholder2,placeholder1)
