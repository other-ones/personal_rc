...
from datasets_pkgs.dataset_cd_partial import CustomDiffusionDataset
...
def collate_fn(examples,with_prior_preservation):
    if 'pixel_values' in examples[0]:
        ...
        is_keyword_tokens = [example["is_keyword_tokens"] for example in examples]
        ...
        is_keyword_tokens_mlm = [example["is_keyword_tokens_mlm"] for example in examples]
    ...
    batch = {
        ...
        "is_keyword_tokens": is_keyword_tokens, # for mlm
        "is_keyword_tokens_mlm": is_keyword_tokens_mlm, # for mlm
...
def main(args):
    ...
    print(hasattr(F, "scaled_dot_product_attention"),'has')
    ...
    for name, _ in unet.attn_processors.items():
        ...
        if cross_attention_dim is not None: # cross_attn
            # print('here1',list(weights.keys()))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states,is_keyword_tokens=is_keyword_tokens).sample
            ...
            #     print(torch.all(initial_cd==cur_cd).item(),'CD same?')
