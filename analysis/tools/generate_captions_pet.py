...
from datasets_pkgs.mlm_pkgs.consts import BACKGROUNDS,STYLES,NONLIVINGS,PET_ACTIVITIES,HUMANS,RELATIVES,NONLIVING_ATTRIBUTES
...
if __name__=='__main__':
    ...
    for location in LOCATIONS:
        for activity in PET_ACTIVITIES:
            prompt3=f"<new> {activity} {location}"
    ...
    for interaction in PET_ACTIVITIES:
        for subject in HUMANS:
            if subject[0] in ['a','e','i','o','u']:
                prompt=f"<new> is {interaction} with an {subject}"
            elif subject[0].isupper():
                prompt=f"<new> is {interaction} with {subject}"
            else:
                prompt=f"<new> is {interaction} with a {subject}"
            captions_pet_interactions.append(prompt)
    # 6. OUTFITS
    captions_pet_outfits=[]
    for outfit in OUTFITS:
        for color in COLORS:
            prompt1=f"<new> wearing {color} {outfit}"
            prompt2=f"<new> in the {color} {outfit}"
