...
if __name__=='__main__':
    ...
    dst_root='../../datasets_pkgs/captions/rc_car'
    ...
    dst_path=os.path.join(dst_root,'captions_rc_car_specific.txt')
    dst_file=open(dst_path,'w')
    captions_rc_car_specific=open("../datasets_pkgs/captions/specific_captions/flower_specific_captions.txt").readlines()
    captions_rc_car_specific=list(set(captions_rc_car_specific))
    captions_rc_car_specific=[item.strip() in captions_rc_car_specific if item.strip()]
    captions_rc_car_specific=sorted(captions_rc_car_specific)
    for caption in captions_rc_car_specific:
        dst_file.write("{}\n".format(caption))
# extend it SIGNIFICANTLY. Hundreds of captions are desired. try to include simple and easy ones as possible
