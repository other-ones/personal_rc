...
for concept in concepts:
    ...
    exps=sorted(exps)[::-1]
    nomlm_clip_score=json.load(open(os.path.join(concept_path,'tiv2_prior_nomlm_{}_s3000'.format(concept),'clip.json')))['clipscore']
    if args.grounded:
        print('{}\t{}\t{}\t{}\t{}'.format(concept,'CLIP','Delta','GDINO','Delta'))
        nomlm_dino_score=json.load(open(os.path.join(concept_path,'tiv2_prior_nomlm_{}_s3000'.format(concept),'{}.json'.format(score_name))))[score_name]
    else:
        print('{}\t{}\t{}\t{}\t{}'.format(concept,'CLIP','Delta','DINO','Delta'))
        nomlm_dino_score=json.load(open(os.path.join(concept_path,'tiv2_prior_nomlm_{}_s3000'.format(concept),'{}.json'.format(score_name))))[score_name]
        print('{}\t{}\t{}\t{}\t{}'.format('tiv2_prior_nomlm_{}_s3000'.format(concept),nomlm_clip_score,'-',nomlm_dino_score,'-'))
    exp_key='tiv2_prior_nomlm_s3000'.format(concept)
    ...
    for exp in exps:
        if exclude is not None and exclude in exp:
            continue
        if 'nomlm_{}_s3000'.format(concept) in exp:
            continue
...
avg_nomlm_clip_score=np.mean(clip_score_dict['tiv2_prior_nomlm_s3000'])
avg_nomlm_dino_score=np.mean(dino_score_dict['tiv2_prior_nomlm_s3000'])
num_nomlm_score=len(clip_score_dict['tiv2_prior_nomlm_s3000'])
...
# assert len(clip_score_dict['tiv2_prior_nomlm_s3000'])==len(dino_score_dict['tiv2_prior_nomlm_s3000'])
# for key in clip_score_dict:
