...
accelerate launch --main_process_port 4244  train_dreambooth_single_mlm.py \
  ...
  --lr_warmup_steps=0 \
  --output_dir="saved_models/dreambooth_models/single/pet_cat1" \
  --seed=7777 \
  --mask_tokens="[MASK]" \
  ...
  --class_prompt="a picture of a cat" \
  --class_data_dir="priors/cat" \
