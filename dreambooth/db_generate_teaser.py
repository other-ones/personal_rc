...
def main(args):
    ...
    exp_dir=args.dst_exp_path # os.path.join(args.output_dir,exp_name)
    ...
    if 'nomlm' not in  args.resume_unet_path:
        mask_tokens = [args.mask_tokens]
        tokenizer.add_tokens(mask_tokens)
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            for iidx,(image, prompt) in enumerate(zip(images[:],prompts[:])):
                ...
                prompt_cat=prompt_cat.replace('.','').replace(',','')
