...
def main():
    ...
    # Load tokenizer
    tokenizer_base = CLIPTokenizer.from_pretrained("stabilityai/stable-diffusion-2-1-base", subfolder="tokenizer")
    text_encoder_base = CLIPTextModel.from_pretrained("stabilityai/stable-diffusion-2-1-base", subfolder="text_encoder", revision=args.revision)

    tokenizer_v2 = CLIPTokenizer.from_pretrained("stabilityai/stable-diffusion-2-1", subfolder="tokenizer")
    text_encoder_v2= CLIPTextModel.from_pretrained("stabilityai/stable-diffusion-2-1", subfolder="text_encoder", revision=args.revision)
    base_state_dict=text_encoder_base.state_dict()
    v2_state_dict=text_encoder_v2.state_dict()
    count=0
    for key in base_state_dict:
        # print(key, key in v2_state_dict)
        p_base=base_state_dict[key]
        p_v2=v2_state_dict[key]
        if torch.all(p_base==p_v2):
            count+=1
        print(torch.all(p_base==p_v2).item(),'same?')
    print(count/len(v2_state_dict),'count/len(v2_state_dict)')
    print(len(base_state_dict),'len(base_state_dict)',len(v2_state_dict),'len(v2_state_dict)')
    exit()

    print(tokenizer_base,'tokenizer_base')
    print(tokenizer_v2,'tokenizer_v2')
    ...
    mask_token_ids_base = tokenizer_base.convert_tokens_to_ids(mask_tokens)
    placeholder_token_id1_base = tokenizer_base.convert_tokens_to_ids(placeholder_token1)
    text_encoder_base.resize_token_embeddings(len(tokenizer_base))

    mask_token_ids_v2 = tokenizer_v2.convert_tokens_to_ids(mask_tokens)
    placeholder_token_id1_v2 = tokenizer_v2.convert_tokens_to_ids(placeholder_token1)
    text_encoder_v2.resize_token_embeddings(len(tokenizer_v2))
    ...
    text_encoder_v2.text_model.encoder.requires_grad_(False)
    text_encoder_v2.text_model.final_layer_norm.requires_grad_(False)
    text_encoder_v2.text_model.embeddings.position_embedding.requires_grad_(False)

    text_encoder_base.text_model.encoder.requires_grad_(False)
    text_encoder_base.text_model.final_layer_norm.requires_grad_(False)
    text_encoder_base.text_model.embeddings.position_embedding.requires_grad_(False)
    ...
    train_dataset_base = TextualInversionDatasetSingle(
    ...
    train_dataset_v2 = TextualInversionDatasetSingle(
        include_prior_concept=args.include_prior_concept,
        data_root1=args.train_data_dir1,
        data_root2=args.train_data_dir2,
