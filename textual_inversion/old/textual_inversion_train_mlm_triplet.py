...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, learned_embed,pipeline):
    logger.info(
    ...
    if args.include_prior_concept:
        placeholder='{} {}'.format(args.placeholder_token,args.prior_concept)
    else:
        placeholder='{}'.format(args.placeholder_token)
    ...
    with autocast_ctx:
        images = pipeline(validation_prompts, num_inference_steps=25, generator=generator).images
...
def main():
    ...
    def collate_fn(examples):
        ...
        input_ids_neg = [example["input_ids_neg"] for example in examples]
        ...
        batch = {
            ...
            "input_ids_neg": input_ids_neg, # for mlm
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(text_encoder):
                ...
                input_ids_neg=batch_mlm["input_ids_neg"].to(accelerator.device)
                ...
                # learned_embeds = text_encoder.get_input_embeddings().weight[min(placeholder_token_ids) : max(placeholder_token_ids) + 1].unsqueeze(0)
                learned_embeds=accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[min(placeholder_token_ids) : max(placeholder_token_ids) + 1].unsqueeze(0)
                # print(learned_embeds.shape,'learned_embeds.shape')
                if args.normalize_target:
                    learned_embeds_scaled=F.normalize(learned_embeds,p=1,dim=-1)*args.target_scale
                    # print(torch.norm(learned_embeds_normalized,p=1,dim=-1))
                    encoder_hidden_states = text_encoder(input_ids,
                                                         is_keyword_tokens=is_keyword_tokens,
                                                         inj_embeddings=learned_embeds_scaled
                                                         )[0].to(dtype=weight_dtype)
                else:
                    encoder_hidden_states = text_encoder(input_ids,
                                                        #  is_keyword_tokens=is_keyword_tokens,
                                                        #  inj_embeddings=learned_embeds
                                                         )[0].to(dtype=weight_dtype)
                ...
                if args.lambda_mlm:
                    ...
                    clip_text_embedding_masked=text_encodings[0].to(accelerator.device)
                    pooled_masked=text_encodings[1].to(accelerator.device)
                ...
                if args.lambda_triplet:
                    tloss=torch.nn.TripletMarginWithDistanceLoss(margin=0.1,
                                                           distance_function=lambda x, y: 1.0 - torch.clip(torch.sum(x*y,dim=1),0,None))
                    pooled_pos = text_encoder(input_ids_pos)[1].to(accelerator.device, dtype=weight_dtype)
                    pooled_neg = text_encoder(input_ids_neg)[1].to(accelerator.device, dtype=weight_dtype)
                    pooled_pos = pooled_pos / torch.sqrt(torch.sum(pooled_neg**2, dim=1, keepdims=True))
                    pooled_neg = pooled_neg / torch.sqrt(torch.sum(pooled_neg**2, dim=1, keepdims=True))
                    pooled_masked = pooled_masked / torch.sqrt(torch.sum(pooled_masked**2, dim=1, keepdims=True))
                    with torch.no_grad():
                        sim_pos = torch.clip(torch.sum(pooled_masked * pooled_pos, dim=1), 0, None)
                        sim_neg= torch.clip(torch.sum(pooled_masked * pooled_neg, dim=1), 0, None)
                    loss_triplet=tloss(anchor=pooled_masked,positive=pooled_pos,negative=pooled_neg)
                    loss+=(loss_triplet*args.lambda_triplet)
                ...
                if ((global_step % args.validation_steps == 0)):
                    ...
                    if not args.debug:
                        ...
                        images = log_validation(
                            text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch,pipeline=pipeline
                        )

                        # save images
                        # validation_files=os.listdir(args.train_data_dir)
                        validation_files=sorted(os.listdir(args.train_data_dir))
                        validation_target=Image.open(os.path.join((args.train_data_dir),validation_files[0])).resize((512,512)).convert('RGB')
                        ...
                        for idx, res in enumerate(images):
                            merged_viz.paste(res.convert('RGB'),((idx+1)*512,0))
            ...
            with torch.no_grad():
                ...
                if args.normalize_target:
                    ...
                    norm_target=torch.norm(learned_embeds,p=1,dim=-1)
