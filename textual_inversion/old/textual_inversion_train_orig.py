...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch):
    ...
    #             }
    #         )
    # merged_viz = Image.new('RGB', (512*len(images), 512), (255, 255, 255))
    # for idx, res in enumerate(images):
    #     merged_viz.paste(res.convert('RGB'),(idx*512,0))
    # print(merged_viz.size,'merged_viz.size')
    # wandb_image = wandb.Image(merged_viz, caption=args.validation_prompt)
    # examples={"examples": [wandb.Image(image, caption=f"{i}: {args.validation_prompt}") for i, image in enumerate(images)]}
    # run.log(examples)
    # accelerator.log({"examples":wandb_image})
...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.exclude_suffix:
            ...
            text = random.choice(self.templates).format(placeholder_string)+' {}'.format(self.target_word)
...
def main():
    args = parse_args()
    dict_args=vars(args)
    run=wandb.init(project='Textual Inversion NonText Orig')
    ...
    # accelerator.init_trackers("Textual Inversion NonText Original", config=dict_args, init_kwargs={"wandb":{"name":args.run_name}})
    ...
    # print(args.gradient_checkpointing,'gradient_checkpointing')
    # exit()
    unet.eval()
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step % args.save_steps == 0) or global_step==1:
                    weight_name = (
                        ...
                        # else 
                        f"learned_embeds_s{global_step}.pt"
                    )
    ...
    if accelerator.is_main_process:
        ...
        # weight_name = "learned_embeds.pt" if args.no_safe_serialization else "learned_embeds.safetensors"
