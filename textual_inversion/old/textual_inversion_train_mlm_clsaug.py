...
def log_validation(tokenizer,  args, accelerator, learned_embed,pipeline):
    logger.info(
    ...
    for prompt in validation_prompts:
        ...
        # print(is_keyword_tokens.shape,'is_keyword_tokens.shape')
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        os.system('cp /home/twkim/anaconda3/envs/char/lib/python3.8/site-packages/transformers/models/clip {} -R'.format(codepath))
    ...
    # Class Augmenter
    from augnet import Augmenter
    augmenter=Augmenter(embed_dim=768*2)
    # init_scale = 0.01
    # for k, v in augmenter.named_parameters():
    #     if 'weight' in k:
    ...
    text_encoder, optimizer, train_dataloader, lr_scheduler,cls_net,mlm_loader,augmenter = accelerator.prepare(
        text_encoder, optimizer, train_dataloader, lr_scheduler,cls_net,mlm_loader,augmenter
    )
    ...
    # exit()
    augmenter.dtype=vae.dtype
    pipeline = StableDiffusionPipelineClsAug(
            ...
            augmenter=accelerator.unwrap_model(augmenter, **extra_args),
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.is_main_process:
                ...
                if ((global_step % args.validation_steps == 0)):
                    ...
                    if not args.debug:
                        # 2. Image Logging
                        # (text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, learned_embed,pipeline)
                        # (tokenizer,  args, accelerator, learned_embed,pipeline)
                        images = log_validation(
                            tokenizer=tokenizer, 
                            args=args, 
                            accelerator=accelerator, 
                            learned_embed=augmented.detach(), 
                            pipeline=pipeline, 
                        )
            ...
            with torch.no_grad():
                target_embeds = accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[min(placeholder_token_ids) : max(placeholder_token_ids) + 1].detach()
                # target_embeds_log=target_embeds.detach()
                # if args.normalize_target:
                #     norm_target=torch.norm(learned_embeds_scaled,p=1,dim=-1)
                # else:
                norm_target=torch.norm(target_embeds,p=1,dim=-1)
                norm_prior=torch.norm(prior_embed,p=1,dim=-1)
            if loss_mlm is not None:
                logs['loss_mlm']=loss_mlm.detach().item()#*args.lambda3
            logs['norm_target']=norm_target.item()
            logs['norm_prior']=norm_prior.item()
            if args.report_to=='wandb' and accelerator.is_main_process:
                wandb.log(logs)
