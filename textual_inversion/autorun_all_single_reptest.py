...
for rep_id in [1,2]:
    ...
    if include_prior:
        dir_name='single_capv7_prior_seed{}_rep_test{}'.format(seed,rep_id)
    else:
        dir_name='single_capv7_noprior_seed{}_rep_test{}'.format(seed,rep_id)
        ...
        for mlm_batch in mlm_batch_list:
            for cidx,concept in enumerate(list(info_map.keys())):
                for mask_prob in mask_prob_list:
                    ...
                    for use_det_alg in use_det_alg_list:
                        device_idx=stat_idx
                        for lambda_mlm in lambda_mlm_list:
                            ...
                            command+='accelerate launch --main_process_port {} ti_train_reptest.py \\\n'.format(ports[cidx])
