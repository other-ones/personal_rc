...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if mask_idxs is not None and mask_embedding is not None:
            # print(inputs_embeds[mask_idxs].shape,'inputs_embeds[mask_idxs].shape')
            # print(mask_embedding.shape,'mask_embedding.shape1')
            # inputs_embeds[mask_idxs]=mask_embedding.repeat(len(inputs_embeds[mask_idxs]),1)
        ...
        if is_keyword_tokens1 is not None and inj_embeddings1 is not None:
            ...
            # print(inputs_embeds[is_keyword_tokens1].shape,'inputs_embeds[is_keyword_tokens1].shape')
            # inputs_embeds[is_keyword_tokens1]=inj_embeddings1.repeat(len(inputs_embeds[is_keyword_tokens1]),1)
...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        # # # # # # # # # # # # #
        # Attn Calibration Params
        # 1) Concept token index
        ...
        if attn_mod_params is not None:
            ...
            if calibrate_kpos1 or calibrate_kneg1:
                ...
                # print(bsz,self.num_heads,tgt_len,'bsz,self.num_heads,tgt_len')
            ...
            # calibration:
            # negative adjustment: score-|score-min_score|
            # positive adjustment: score+|max_score-score|
            if calibrate_kneg1:
                # k1 - neg
        ...
        # print(attn_weights[is_keyword_tokens1].argmax(-1),'attn_weights[is_keyword_tokens1].argmax(-1)_re1')
        # apply the causal_attention_mask first
        if causal_attention_mask is not None: #HERE
            # print('causal_attention_mask is not None')
        ...
        # print(attn_weights[is_keyword_tokens1].argmax(-1),'attn_weights[is_keyword_tokens1].argmax(-1)_re2')
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        

        # k1_probs=attn_weights[is_keyword_tokens1].view(bsz * self.num_heads, tgt_len) # 4800,77
        # print(k1_probs.argmax(-1),'k1_probs.argmax')
        # k1_p1_probs=k1_probs[is_prior1]
        # k1_sot_probs=k1_probs[:,0]
        # print(k1_p1_probs,'k1_p1_probs')
        # print(k1_sot_probs,'k1_sot_probs')
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        # # HERE
        # else:
        #     print(add_pe,'add_pe HERE2')
        #     hidden_states = self.embeddings(input_ids=input_ids, 
        #                                 position_ids=position_ids,
        #                                 )
        #     if mask_idxs is not None and mask_embedding is not None:
        #         # print(hidden_states[mask_idxs].shape,'hidden_states[mask_idxs].shape')
        #         # print(mask_embedding.shape,'mask_embedding.shape')
        #         hidden_states[mask_idxs]=mask_embedding[0].repeat(len(hidden_states[mask_idxs]),1)
        #     if is_keyword_tokens1 is not None and inj_embeddings1 is not None:
        #         # print(inj_embeddings1.shape,'inj_embeddings1.shape')
        #         # print(hidden_states[is_keyword_tokens1].shape,'hidden_states[is_keyword_tokens1].shape')
        #         hidden_states[is_keyword_tokens1]=inj_embeddings1.repeat(len(hidden_states[is_keyword_tokens1]),1)
        #     if is_keyword_tokens2 is not None and inj_embeddings2 is not None:
