...
exp_prefix='abl_prompt_type'
if include_prior:
    dir_name=f'{exp_prefix}_prior_seed{seed}_qlab{host_suffix}_rep{rep_id}'
else:
    dir_name=f'{exp_prefix}_noprior_seed{seed}_qlab{host_suffix}_rep{rep_id}'
train_steps=3001
mlm_batch=25
# prompt_size_list=[1,10,100]
exc_list=['backgrounds','styles','wearing','relations']
for exc in exc_list:
    exclude_cap_types=exc
        ...
        for mlm_target in mlm_target_list:
            for cidx,concept in enumerate(list(info_map.keys())):
                ...
                for mask_prob in mask_prob_list:
                    ...
                    prefix=f'ti_{exp_prefix}_qlab{host_suffix}'
                    if include_prior:
                        prefix+='_prior'
                    else:
                        prefix+='_noprior'
                    ...
                    while True:
                        ...
                        time.sleep(delay)
                    print(f"{dir_name}\t{run_name}\tDEVICE:{device_idx}")
