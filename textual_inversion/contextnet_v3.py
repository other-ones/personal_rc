...
class ContextNetV3(nn.Module):
    def __init__(
        ...
        self.feed_forward2 = FeedForward(embed_dim=in_channels)

        self.multi_head_attention3 = nn.MultiheadAttention(in_channels, num_heads=4, batch_first=True)
        self.layer_norm3 = nn.LayerNorm(in_channels)
        self.feed_forward3 = FeedForward(embed_dim=in_channels)

        self.multi_head_attention4 = nn.MultiheadAttention(in_channels, num_heads=4, batch_first=True)
        self.layer_norm4 = nn.LayerNorm(in_channels)
        # for _ in range(4):
